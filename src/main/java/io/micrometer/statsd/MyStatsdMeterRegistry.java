...
import io.micrometer.statsd.internal.MyBufferingFlux;
 ...
 * @since 1.0.0
 */
public class MyStatsdMeterRegistry extends MeterRegistry {
  ...
  public MyStatsdMeterRegistry(StatsdConfig config, Clock clock) {
   ...
   */
  public MyStatsdMeterRegistry(StatsdConfig config, HierarchicalNameMapper nameMapper, Clock clock) {
    this(config, nameMapper, namingConventionFromFlavor(config.flavor()), clock, null, null);
  }

  private MyStatsdMeterRegistry(StatsdConfig config,
  ...
  public void start() {
    if (started.compareAndSet(false, true)) {
      ...
      if (lineSink != null) {
        ...
        final Flux<String> bufferingPublisher = MyBufferingFlux
          .create(Flux.from(processor), "\n", statsdConfig.maxPacketLength(), statsdConfig.pollingFrequency().toMillis())
  ...
  /**
   * A builder for configuration of less common knobs on {@link MyStatsdMeterRegistry}.
   */
  ...
  public static class Builder {
    ...
    public MyStatsdMeterRegistry build() {
      return new MyStatsdMeterRegistry(config, nameMapper, namingConvention, clock, lineBuilderFunction, lineSink);
    }
  }
